...
| 拦截语音键附带的键盘信号 | 连接期间拦截 F5，以及内核驱动转换后的 F20，防止干扰输入法快捷键，默认开 |
...
| `MiVoiceMic.exe --selftest` | 离线单元测试（解码器/分帧/配置/热键/手势引擎/按键映射/充电状态解析/按键归因/会话恢复/蓝牙超时，99 项） |
...
| `blockF5` | `true` | 拦截语音键的 F5 / 驱动转换后的 F20（仅连接期间生效，兼容原配置名） |
...
若测试热键正常、按遥控器却不出现语音条，尤其是在安装内核驱动后，请保持“拦截语音键附带的键盘信号”开启。驱动会把语音键的 F5 改成 F20；漏掉 F20 会使输入法收到额外的键，干扰 Ctrl+Win 等组合。Windows 按注册顺序调用键盘拦截器，后注册的先收到事件，因此仅看到“累计拦截语音键事件”增长，并不能证明输入法没有先收到它。程序在触发前刷新拦截顺序，先安装新拦截器再移除旧拦截器，并补发 F5/F20 松开信号清理已被其他程序记录的按下状态。发送快捷键期间继续拦截真实语音键，模拟按键正常放行。
