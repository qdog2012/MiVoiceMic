...
从 v1.0.3 起，打开 **关于 → 软件更新** 会自动检查 GitHub 最新正式版本，也可点击 **检查更新** 重试。有新版时点击 **下载并更新**，程序将下载并校验 EXE，退出后完成替换并自动重启。下载期间可以取消，现有配置和驱动保持原样。网络异常或校验失败时会停止更新；替换失败时保留原程序，启动失败时尝试恢复旧程序。旧版备份保存在程序目录下隐藏的 `.MiVoiceMic-update-*` 文件夹中的 `previous.exe`。

程序目录需要可写；若提示权限不足，请以管理员身份运行后重试。只有版本号高于本机、带有校验信息的正式发布支持自动更新；预发布版不会安装。
...
打开 **连接与语音 → 音频与增益 → 选择音频设备**，从下拉列表中选择同一条虚拟声卡的播放端和录音端，再点击 **应用并保存**。部分驱动的播放端叫 **CABLE In 16 Ch**，请以本机列表为准。更换设备后可点击 **刷新设备**；设置立即生效，无需重启程序。请先松开语音键再应用。

选定设备后会保存设备标识，设备改名或列表顺序变化不会误选其他设备。旧配置仍支持名称匹配；原设备缺失或名称不唯一时，窗口会提示重新选择，环境自检也会报告配置的设备不可用。应用失败时保留原设置。
...
启用“开机自启”后，登录 Windows 时只显示右下角托盘图标，不弹出主界面。手动双击 EXE 会显示主界面；若程序已经在托盘运行，会直接打开已有窗口，不会重复启动或重复连接遥控器。双击托盘图标也可打开主界面。已有的开机自启设置会在新版首次运行时自动迁移。
...
| **环境自检** | 蓝牙适配器 / 遥控器配对 / 遥控器当前连接（蓝牙连通与语音通道就绪分开显示，每 3 秒刷新）/ VB-CABLE 两端 / 微信输入法 逐项检查。`--check` 也检查系统蓝牙连接，语音通道状态以运行中的主程序为准。 |
| **关于** | 当前版本、GitHub 更新检查与一键下载安装、致谢、config/log 快捷方式、运行日志实时尾部 |
...
系统托盘图标直接显示遥控器电量数字（例如 `63` 表示 63%）。正常为蓝色，电量 16–30% 为橙色、0–15% 为红色，充电时为绿色；尚未读到电量时显示 `?`，断开连接时显示 `×`。悬停可查看百分比、充电状态和连接状态。图标随遥控器上报或程序定时读取的电量更新；若图标被 Windows 折叠，可将其拖到任务栏通知区域常驻显示。
...
| 拦截语音键附带的键盘信号 | 默认开；驱动运行时只拦截语音键 F20，放行普通键盘 F5；未运行驱动时使用 F5/F20 兼容拦截 |
...
| `MiVoiceMic.exe --selftest` | 离线单元测试（解码器/分帧/配置/热键/手势引擎/按键映射/充电状态解析/按键归因/会话恢复/蓝牙超时/音频设备选择/更新，157 项） |
...
| `cableRenderName` / `cableCaptureName` | `CABLE Input` / `CABLE Output` | 播放端 / 录音端名称；旧配置按名称匹配，可在“选择音频设备”中设置 |
| `cableRenderId` / `cableCaptureId` | 空 | 界面选择后保存的设备标识，优先于名称；设备缺失时需重新选择 |
| `switchLeadMs` | `120` | 实际切换默认麦克风后、注入热键前的等待；默认麦克风已是 CABLE 时跳过 |
| `agc` | `true` | 自适应增益（推荐开）；关闭时用固定 `gainDb`（-24…24 dB） |
| `blockF5` | `true` | 连接期间拦截语音键；驱动运行时只拦截 F20，未运行驱动时兼容 F5/F20，配置名保持兼容 |
...
若测试热键正常、按遥控器却不出现语音条，尤其是在安装内核驱动后，请保持“拦截语音键附带的键盘信号”开启。驱动会把语音键的 F5 改成 F20；漏掉 F20 会使输入法收到额外的键，干扰 Ctrl+Win 等组合。Windows 按注册顺序调用键盘拦截器，后注册的先收到事件，因此仅看到“累计拦截语音键事件”增长，并不能证明输入法没有先收到它。程序在触发前刷新拦截顺序，先安装新拦截器再移除旧拦截器，并补发语音键松开信号清理残留状态：驱动运行时只清理 F20，未运行驱动时兼容 F5/F20。发送快捷键期间继续拦截真实语音键，模拟按键正常放行。

日志中的热键发送 `2/2` 只表示 Windows 接受了注入，不代表输入法已启动。需要以输入框中实际出现语音条为准。需要进一步诊断时，`tools/voice-key-trace.cs` 可编译为单独的只读记录工具，参数为记录秒数和输出路径；它仅记录 F5/F20 与左右 Ctrl/Win/Alt/Shift，不记录普通文字键。

**语音识别到的是电脑麦克风的声音 / 没声音**
先打开 **连接与语音 → 选择音频设备**，确认播放端和录音端都存在并属于同一条虚拟声卡，然后应用。若本机只有 `CABLE In 16 Ch`，配置为 `CABLE Input` 会导致输出打不开。说话图标只代表热键已触发；调试 WAV 保存的是解码后的声音，未经过虚拟声卡，两者正常都不能证明输入法收到了音频。
...
**普通键盘 F5 被拦截**
程序会在启动和遥控器连接时检查 `MiRemoteHidFilter` 是否实际运行。驱动运行时，遥控器语音键已经转换为 F20，普通键盘 F5 会直接放行，语音开始前也不会额外释放 F5。日志会显示“语音键使用 F20，普通键盘 F5 放行”。安装或卸载驱动后可重新连接遥控器刷新检测。未运行驱动时仍采用旧的 F5/F20 兼容拦截，会影响物理 F5；可临时在“连接与语音 → 遥控器行为”关闭拦截，但此时遥控器语音键也会传入前台应用。遥控器未连接时物理 F5 不受影响。
