...
双击 `MiVoiceMic.exe`：主窗口（标准 Windows 窗口，可拖动、可调整大小）+ 托盘图标同时出现。连接成功后设备卡显示 **已连接** 和电量：
...
映射引擎通过 **Raw Input 设备归因**（按 HID VID:PID，默认 `2717:32B8`）区分遥控器按键与物理键盘——物理键盘的相同按键不受影响。带长按手势的键在阈值（默认 600ms）前后分别触发点按/长按动作。
...
| `MiVoiceMic.exe --selftest` | 离线单元测试（解码器/分帧/配置/热键/手势引擎/按键映射，31 项） |
...
| `switchLeadMs` | `120` | 切完默认麦克风后、注入热键前的等待（部分应用需要时间感知设备变化） |
...
遥控器固件偶发卡住：重启应用（托盘→退出）。应用本身会自动重连并重试握手。
